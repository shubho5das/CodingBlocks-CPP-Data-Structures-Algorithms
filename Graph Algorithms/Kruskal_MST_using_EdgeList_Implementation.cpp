/*
SPOJ: MST - Minimum Spanning Tree 
			(using Kruskal's)

Time Complexity: O(ElogE + ElogV)
*/

#include<bits/stdc++.h>
using namespace std;

#define ll long long
#define ld long double
#define mod 1000000007 // 1e9+7
#define inf 1e18

#define watch(x) cout<<(#x)<<" is "<<(x)<<"\n"
#define watch2(x,y) cout<<(#x)<<" is "<<(x)<<" and "<<(#y)<< " is " <<(y)<<"\n"

class edge{
public:
	ll u;
	ll v;
	ll w;

	// Constructor
	edge(ll a,ll b,ll c){
		u = a;
		v = b;
		w = c;
	}
};

bool edge_compare(edge a,edge b){
	return(a.w < b.w);
}

class graph{

private:
	ll V;
	vector<edge> L; // Edge-List

	/* NOTE:	
	During DSU implementation,
		V = 4 implies, Vertices are {0,1,2,3}.
	Hence, it is necesary to maintain 0-indexing of the vertices.
	
	
	If in some Problem, we are instead provided 1-indexed 
	vertices in the sample input,
		then, we must convert 1-indexing -> 0-indexing :

	 - keep doing u -= 1,v -= 1; while adding edges to the Graph,
	 - while processing querries of nodes also, 
	 	do src -= 1, dest -= 1, .. and so on. 
	 */  	

public:

	// Constructor
	graph(ll x){
		V = x;
	}

	void add_edge(ll u,ll v,ll w){
		edge e(u,v,w);
		L.push_back(e); 
	}

	// DSU Functions

	ll find_set(ll i,ll parent[]){
		// Path Compression Optimisation.
		
		// Base Case
		if(parent[i] == -1)
			return i;

		// Recursive Case
		parent[i] = find_set(parent[i],parent);
		return find_set(parent[i],parent);
	}

	void union_set(ll x,ll y,ll parent[],ll rank[]){
		// Union by Rank Optimisation
		
		ll S1 = find_set(x,parent);
		ll S2 = find_set(y,parent);

		if(S1 != S2){
			if(rank[S1] < rank[S2]){
				parent[S1] = S2;
				rank[S2] += rank[S1];
			}
			else{
				parent[S2] = S1;
				rank[S1] += rank[S2];
			}
		}
	}


	ll kruskal_MST(){

		ll i,j,wt,cost,S1,S2;


		// Step 1: Sort edges in increasing order of their wts.
		sort(L.begin(),L.end(),edge_compare);

		// Step 2: Keep adding edges, which don't lead to cycle.

		// DSU Approach
		ll parent[V];
		ll rank[V];
		for(i=0 ; i<V ; i++){
			parent[i] = -1;
			rank[i] = 1;
		}
		
		cost = 0;			
		for(auto current_edge : L){ 

			i = current_edge.u;
			j = current_edge.v;
			wt = current_edge.w;

			S1 = find_set(i,parent);
			S2 = find_set(j,parent);

			if(S1 != S2){
				union_set(S1,S2,parent,rank);
				cost += wt;
			}
		}

		return cost;

	}
};

int main(){

    ios_base::sync_with_stdio(false);
    cin.tie(NULL);

    ll n,m,i,u,v,w;

    cin>>n>>m;

    graph G(n);

    for(i=0 ; i<m ; i++){
    	cin>>u>>v>>w;
    	u -= 1;
    	v -= 1;
    
    	G.add_edge(u,v,w);
    }

    cout<<G.kruskal_MST()<<"\n";
    
    return 0;
}